...
class tc_pa_063(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_063, self).__init__(driver)
    ...
    def refProductSelect(self):
        wait = WebDriverWait(self.driver, 60)
        product = wait.until(
            EC.visibility_of_element_located((By.XPATH, "(//label[normalize-space()='DSL Cash'])[1]")))
        product.click()
    ...
    def referenceCalcType(self):
        wait = WebDriverWait(self.driver, 60)
        calc = wait.until(EC.visibility_of_element_located((By.XPATH, "(//label[normalize-space()='Minimum'])[1]")))
        calc.click()
