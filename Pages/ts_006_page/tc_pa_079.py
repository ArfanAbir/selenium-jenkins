...
class tc_pa_079(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_079, self).__init__(driver)
    ...
    def premModelCashPlus(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[6]")))
        model.click()

    def premModelCashPlusAgain(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[6]")))
        model.click()

    def premModelCashMinus(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[6]")))
        model.click()