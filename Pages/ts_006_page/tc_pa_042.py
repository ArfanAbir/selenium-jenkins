...
class tc_pa_042(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_042, self).__init__(driver)
    ...
    def stationaNameFilter(self):
        wait = WebDriverWait(self.driver, 60)
        name = wait.until(EC.element_to_be_clickable((By.XPATH, "//a[@id='station_filter']")))
        name.click()

    def selectAll(self):
        wait = WebDriverWait(self.driver, 60)
        select = wait.until(EC.element_to_be_clickable((By.XPATH, "//a[@id='m-sall-table']")))
        select.click()
    ...
    def clearButton(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        clear = wait.until(EC.element_to_be_clickable((By.XPATH, "//a[@id='m-clear-table']")))
        clear.click()

    def cancelButton(self):
        wait = WebDriverWait(self.driver, 60)
        submit = wait.until(EC.element_to_be_clickable((By.XPATH, "//button[@id='m-cancel-table']")))
        submit.click()