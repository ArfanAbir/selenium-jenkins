...
class tc_pa_082(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_082, self).__init__(driver)
    ...
    def scrollDown(self):
        ...
        scrollPoint = wait.until(
            EC.visibility_of_element_located((By.XPATH, "//th[normalize-space()='e85']")))
        self.driver.execute_script("arguments[0].scrollIntoView();", scrollPoint)

    def e85ModelCashPlus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[8]")))
        model.click()

    def e85ModelCashPlusAgain(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[8]")))
        model.click()

    def e85ModelCashMinus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[8]")))
        model.click()