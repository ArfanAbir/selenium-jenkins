...
class tc_pa_084(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_084, self).__init__(driver)
    ...
    def dieselModelCreditPlus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[9]")))
        model.click()

    def dieselModelCreditPlusAgain(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[9]")))
        model.click()

    def dieselModelCreditMinus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[9]")))
        model.click()