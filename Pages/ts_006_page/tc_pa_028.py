...
class tc_pa_028(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_028, self).__init__(driver)
    ...
    def marginalNonFuel(self):
        wait = WebDriverWait(self.driver, 60)
        swipe = wait.until(EC.element_to_be_clickable((By.XPATH, "(//*[name()='text'][@class='highcharts-title'])[13]")))
        swipe.click()

    def zoom1m(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        zoom = wait.until(EC.element_to_be_clickable((By.XPATH, "(//*[name()='text'][contains(text(),'1m')])[2]")))
        zoom.click()

    def marginNonFuelUncheck(self):
        wait = WebDriverWait(self.driver, 60)
        verify = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Margin non Fuel'])[1]")))
        self.driver.execute_script("arguments[0].click();", verify)

    def marginNonFuelCheck(self):
        wait = WebDriverWait(self.driver, 60)
        verify = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Margin non Fuel'])[1]")))
        self.driver.execute_script("arguments[0].click();", verify)