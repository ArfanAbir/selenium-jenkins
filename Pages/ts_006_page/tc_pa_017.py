...
class tc_pa_017(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_017, self).__init__(driver)
    ...
    def productSelectREG(self):
        wait = WebDriverWait(self.driver, 60)
        product = wait.until(EC.element_to_be_clickable((By.XPATH, "(//label[normalize-space()='REGULAR'])[1]")))
        product.click()

    def productSelectAllState(self):
        wait = WebDriverWait(self.driver, 60)
        product = wait.until(EC.element_to_be_clickable((By.XPATH, "(//label[normalize-space()='Select all'])[1]")))
        product.click()
    ...
    def timeSelect(self):
        wait = WebDriverWait(self.driver, 60)
        time = wait.until(EC.element_to_be_clickable((By.XPATH, "(//option[@value='m'])[1]")))
        time.click()

    def state(self):
        wait = WebDriverWait(self.driver, 60)
        state = wait.until(EC.element_to_be_clickable((By.XPATH, "(//input[@id='chkSelectAll'])[1]")))
        state.click()
