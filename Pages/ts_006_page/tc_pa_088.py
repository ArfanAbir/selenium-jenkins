...
class tc_pa_088(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_088, self).__init__(driver)
    ...
    def stationManagerState(self):
        wait = WebDriverWait(self.driver, 60)
        station = wait.until(
            EC.visibility_of_element_located((By.XPATH,
                                              "//span[@class='statesName tradeclusters'][normalize-space()='New York']")))
        station.click()
    ...
    def deleteConfirm(self):
        wait = WebDriverWait(self.driver, 60)
        delete = wait.until(EC.element_to_be_clickable((By.XPATH, "//button[normalize-space()='confirm']")))
        self.driver.execute_script("arguments[0].click();", delete)
