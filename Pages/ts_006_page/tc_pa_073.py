...
class tc_pa_073(HomePage):
    ...
    def cashModelPlusIcon(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        plus = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[2]")))
        plus.click()

    def cashModelPlusAgain(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        plus = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[2]")))
        plus.click()

    def cashModelMinusIcon(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        plus = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[2]")))
        plus.click()
