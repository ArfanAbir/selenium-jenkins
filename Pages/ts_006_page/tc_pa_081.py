...
class tc_pa_081(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_081, self).__init__(driver)
    ...
    def e85ModelCreditPlus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[7]")))
        model.click()

    def e85ModelCreditPlusAgain(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[7]")))
        model.click()

    def e85ModelCreditMinus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[7]")))
        model.click()