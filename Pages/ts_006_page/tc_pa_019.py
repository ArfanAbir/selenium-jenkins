...
class tc_pa_019(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_019, self).__init__(driver)
    ...
    def uncombineAllEffect(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        uncombine = wait.until(
            EC.visibility_of_element_located((By.XPATH, "//button[normalize-space()='Uncombine All Effects']")))
        self.driver.execute_script("arguments[0].click();", uncombine)

    def combineAllEffect(self):
        wait = WebDriverWait(self.driver, 60)
        combine = wait.until(
            EC.visibility_of_element_located((By.XPATH, "(//button[normalize-space()='Combine All Effects'])[1]")))
        self.driver.execute_script("arguments[0].click();", combine)