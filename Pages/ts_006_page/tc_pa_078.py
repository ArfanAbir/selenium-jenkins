...
class tc_pa_078(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_078, self).__init__(driver)
    ...
    def premModelCreditPlus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[5]")))
        model.click()

    def premModelCreditPlusAgain(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[5]")))
        model.click()

    def premModelCreditMinus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        model = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[5]")))
        model.click()