...
class tc_pa_016(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_016, self).__init__(driver)
    ...
    def productSelectNonFuel(self):
        wait = WebDriverWait(self.driver, 60)
        product = wait.until(EC.element_to_be_clickable((By.XPATH, "(//label[normalize-space()='NON FUEL'])[1]")))
        product.click()
    ...
    def timeSelect(self):
        wait = WebDriverWait(self.driver, 60)
        time = wait.until(EC.element_to_be_clickable((By.XPATH, "//option[@value='d']")))
        time.click()

    def state(self):
        wait = WebDriverWait(self.driver, 60)
        state = wait.until(EC.element_to_be_clickable((By.XPATH, "(//a[normalize-space()='New York'])[1]")))
        state.click()
