...
class tc_pa_076(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_076, self).__init__(driver)
    ...
    def midModelCashPlus(self):
        wait = WebDriverWait(self.driver, 60)
        credit = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[4]")))
        credit.click()

    def midModelCashPlusAgain(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        credit = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'+')])[4]")))
        credit.click()

    def midModelCashMinus(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        credit = wait.until(EC.element_to_be_clickable((By.XPATH, "(//span[contains(text(),'-')])[4]")))
        credit.click()