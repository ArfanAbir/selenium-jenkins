...
class tc_pa_051(HomePage):
    ...
    def cpgEqual(self):
        wait = WebDriverWait(self.driver, 60)
        drop = wait.until(EC.element_to_be_clickable((By.XPATH, '//*[@id="m-list-table"]/div/div[1]/select/option[3]')))
        drop.click()
    ...
    def checkValue(self):
        ...
        if val.text >= "5":
            print("Your New Value is Equal to  30")
    ...
    def removeSearch(self):
        wait = WebDriverWait(self.driver, 60)
        remove = wait.until(EC.element_to_be_clickable((By.XPATH, "//i[contains(@title,'Remove')]")))
        remove.click()
