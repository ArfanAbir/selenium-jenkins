...
class tc_pa_029(HomePage):

    def __init__(self, driver):
        self.driver = driver
        super(tc_pa_029, self).__init__(driver)
    ...
    def revenueFuel(self):
        time.sleep(2)
        wait = WebDriverWait(self.driver, 60)
        swipe = wait.until(EC.element_to_be_clickable((By.XPATH, "(//*[name()='text'][@class='highcharts-title'])[14]")))
        swipe.click()

    def revZoom6m(self):
        wait = WebDriverWait(self.driver, 60)
        zoom = wait.until(EC.element_to_be_clickable((By.XPATH, "(//*[name()='text'][contains(text(),'6m')])[3]")))
        zoom.click()

    def revZoom1m(self):
        wait = WebDriverWait(self.driver, 60)
        zoom = wait.until(EC.element_to_be_clickable((By.XPATH, "(//*[name()='text'][contains(text(),'1m')])[3]")))
        zoom.click()

    def revRegularUncheck(self):
        wait = WebDriverWait(self.driver, 60)
        reg = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Regular'])[1]")))
        self.driver.execute_script("arguments[0].click();",reg)

    def revMidUncheck(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        mid = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Mid-Grade'])[1]")))
        self.driver.execute_script("arguments[0].click();",mid)

    def revPremUncheck(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        prem = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Premium'])[1]")))
        self.driver.execute_script("arguments[0].click();",prem)

    def revDslUncheck(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        dsl = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Diesel'])[1]")))
        self.driver.execute_script("arguments[0].click();", dsl)

    def revRegularCheck(self):
        wait = WebDriverWait(self.driver, 60)
        reg = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Regular'])[1]")))
        self.driver.execute_script("arguments[0].click();", reg)

    def revMidCheck(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        mid = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Mid-Grade'])[1]")))
        self.driver.execute_script("arguments[0].click();", mid)

    def revPremCheck(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        prem = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Premium'])[1]")))
        self.driver.execute_script("arguments[0].click();", prem)

    def revDslCheck(self):
        time.sleep(3)
        wait = WebDriverWait(self.driver, 60)
        dsl = wait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@aria-label='Show Diesel'])[1]")))
        self.driver.execute_script("arguments[0].click();", dsl)